...
object ServiceLocator {
    ...
    var tasksRepository: TasksRepository? = null
        @VisibleForTesting set //it can be setter in a test class
    ...
    private fun createTaskRepository(context: Context): TasksRepository {
        val repo = DefaultTasksRepository(TasksRemoteDataSource, createLocalDb(context))
        tasksRepository = repo
        return repo
    }

    private fun createLocalDb(context: Context): TasksDataSource {
        val db = database ?: createDb(context)
        return TasksLocalDataSource(db.taskDao())
    }

    private fun createDb(context: Context): ToDoDatabase {
